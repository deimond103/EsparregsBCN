...
def set_led(color):
    ...
    for led, path in leds.items():
        try:
            with open(path, 'w') as f:
                f.write(str(states[color][led]))
        except Exception as e:
            print(f"[LED] Error setting {led}: {e}")

def update_led_from_severity(severity):
    if severity is None:
        set_led('green')
    elif severity == 'low':
        set_led('blue')
...
def send_detections_to_ui(detections: dict):
    ...
    severity = get_severity(person_count)
    update_led_from_severity(severity)
